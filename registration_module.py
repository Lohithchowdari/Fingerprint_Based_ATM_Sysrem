...
def Mini_statement(acc_no):
    os.system("cls")
    # Changed to use ATM_database instead of transaction_sheet
    mini_statement_sql_query = """select Account_no, Balance, Transaction, Amount, Date_Time 
                                  from ATM_database where Account_no = %s"""
    connection_cursor.execute(mini_statement_sql_query,(acc_no,))
    mini_statement_search_result = connection_cursor.fetchall()
    
    if mini_statement_search_result:
        row = mini_statement_search_result[0]
        ...
        print("No transaction records found for account:", acc_no)
    exit()